...
module.exports = class OAuth {
  ...
  // Middleware function, determines various info to pass
  ...
  static checkPermissions (res, guild) {
    // There is no guild?
    if (!guild) {
  ...
  static async updateUser (app, userID) {
    ...
    // Get new user data
    const user = await app.locals.rest.api.users('@me').get(null, null, `Bearer ${token.access_token}`);
    const guilds = await app.locals.rest.api.users('@me').guilds.get(null, null, `Bearer ${token.access_token}`);
  ...
  static async findGuild (app, data, id) {
    ...
    if (!userData) {
      let { guilds } = await OAuth.updateUser(app, data.userID);
